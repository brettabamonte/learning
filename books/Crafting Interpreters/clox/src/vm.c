...
static void runtimeError(const char* format, ...) {
    ...
    //Init i to -1 b/c we want the previously failed instruction
...
static bool call(ObjClosure* closure, int argCount) {
    ...
    frame->slots = vm.stackTop - argCount - 1; // -1 accounts for VM taking slot 0
    return true;
}
    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_CLASS: {
                ...
                Value initalizer;
                if (tableGet(&klass->methods, vm.initString, &initalizer)) {
...
static ObjUpvalue* captureUpvalue(Value* local) {
    ...
    createdUpvalue->next = upvalue; //very important. Will break chain of upvalues. More than 1 upvalue and only the last upvalue will be resolveable.
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_RETURN: {
                ...
                vm.stackTop = frame->slots; //discard slots callee was using for parameters & local variables
                push(result);
                frame = &vm.frames[vm.frameCount - 1];
                break;
            }
